...
- Use route-level `lazy` in the data router, never bare `React.lazy`, because the
  parity suite asserts immediately after navigation.
- Tolerate `null` and non-array response bodies on every read.
- Use `sanitizeHtml` from `web/src/lib/html.ts`; never use raw
  `dangerouslySetInnerHTML`.
- Guard every `window.ga` call because analytics is optional.
...
- meta description / twitter:_ / og:_ / fb:admins / viewport / theme-color(#b92b27 then #1976d2)
  / msapplication-_ / apple-mobile-web-app-_
...
- Production PWA service worker precaches index/css/js/manifest and the static
  assets; it is not generated or registered during development.
