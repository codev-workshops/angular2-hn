...
The app uses React 19, Vite 6, react-router, zustand, Vitest, and Playwright.
Modern Node.js is required.
...
yarn build                 # production output in dist-react/
yarn test
yarn typecheck
yarn lint
yarn test:e2e
...
`yarn format` and `yarn format:check` format and verify the source and project
configuration files. The PWA service worker is generated only for production
builds and is never registered by the development server.

The project layout keeps the React application in `web/`, static public assets
in `web/public/`, and the frozen Playwright parity suite in `e2e/playwright/`.
Firebase Hosting serves `dist-react/` and rewrites application routes to
